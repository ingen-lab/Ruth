...
//Global declarations
string cAnimation = "";      //the currently selected animation from the HUD inventory
string cPrevRtAnim = "";     //the previously selected animation on the right side
string cPrevLfAnim = "";     //the previously selected animation on the left side
string cWhichSide = "";      //Whether the currently selected animation is a right or left hand
key gOwner;
integer nButtonNo = 0;       //The number of the button pressed by the user
integer nPrimNo;             //The prim which contains the button pressed
integer nPrimFace;           //The face of the prim containing the button pressed
integer nButtonStart;        //The starting number of a group of buttons
vector vONColor =  <0.224, 0.800, 0.800>;  //Teal color to indicate the button has been pressed
vector vOFFColor = <1.0, 1.0, 1.0>;  //white color to indicate the button has not been pressed
integer lStopAll =FALSE;
...
ColorButton()  
{
    cWhichSide ="";  //initialize     
    if(nPrimFace & 0x01) //check to see whether it is odd (all "right" buttons) - faces start at 0 so all odd numbers are right buttons
    {
        cWhichSide = "RIGHT" ; // odd number
    }
    else  //or even (all "left" buttons
    {
        cWhichSide = "LEFT"; // even number
    }
    ...
    {        
        ...
        do //cycle through each face (contained on each prim)
        {
            ...
            if((nPrimCount==nPrimNo) && (nFaceCount==nPrimFace)) //if this is the button being pressed
            {
                llSetLinkPrimitiveParamsFast(nPrimNo, [PRIM_COLOR, nPrimFace, vONColor, 1.0]); //color the button
            }
            else
            {
                if(cWhichSide=="RIGHT") 
                {
                    ...
                    {             
                        llSetLinkPrimitiveParamsFast(nPrimCount, [PRIM_COLOR, nFaceCount, vOFFColor, 1.0]); //remove color from the button
                    }
                }
                else  //lLeftButtons
                {
...
default
{
    touch_start(integer detected)
    {
        ...
        if (llDetectedKey(0) == llGetOwner())
        {
            ...
            nPrimNo = llDetectedLinkNumber(0);
            nPrimFace = llDetectedTouchFace(0); //note that first face in a prim is 0 not 1
            list linkParamList = llGetLinkPrimitiveParams(nPrimNo,[PRIM_NAME, PRIM_DESC, PRIM_TEXTURE,-1]);
            string cPrimName = llList2String(linkParamList,0); //returns the prim name of the set of buttons touched           
            nButtonStart = (integer)cPrimName;  //cPrimName contains the starting number for that set of buttons            
            nButtonNo = nButtonStart + nPrimFace;

            if ((nPrimNo!=1) && (nPrimFace < 6))  //Prim 1 is the root prim (backboard & we skip it). Also can't have any faces greater than 5 on the button prims
            {         
    ...
    run_time_permissions(integer perm)
    {
        if (perm & PERMISSION_TRIGGER_ANIMATION)
            ...
            if (lStopAll)  //User pressed the Stop All Animations button
            {
                lStopAll=FALSE;            
            ...
            {            
                ...
                {
                    nCounter++;
                    cAnimation = llGetInventoryName(INVENTORY_ANIMATION, nCounter);  //note that first animation on list is 0 not 1               
                    nItemNo = (integer)cAnimation;   //Each animation has a number, ie: 02_HandRelaxed1                
                    if (nItemNo == nButtonNo)  //When the Animation number matches the button number 
                    {                   
                        if(cAnimation != "")
                        {
                            ColorButton();  //this runs the user defined function which turns the buttons on & off
                                            //it also returns a value for cWhichSide 
                            
                            if(cWhichSide=="RIGHT") //stop the previous animation on the same side as the new one                                                   
                            {   
                                if(cPrevRtAnim != "")
                                {                                                           
                                    llStopAnimation(cPrevRtAnim);
                                }
                                cPrevRtAnim = cAnimation;                                    
                            }    
                            else if (cWhichSide == "LEFT")
                            {
                                if(cPrevLfAnim != "")
                                {                                                           
                                    llStopAnimation(cPrevLfAnim);
                                }
                                cPrevLfAnim = cAnimation;
                            }                            
                            llStartAnimation(cAnimation);  //start the user's selected animation                        
                            //llOwnerSay("We started: "+cAnimation+"  cPrevLfAnim is: "+cPrevLfAnim+"  " + "cPrevRtAnim is: "+cPrevRtAnim);
                ...
                if (!(nButtonStart==1 | nButtonStart==7 | nButtonStart == 13 | nButtonStart ==19))  //error trapping
                {
                   nItemNo = 100;  //temporarily set this to 100 to trigger the proper error message
                   lFlag = FALSE; 
                }   
                
                if(! lFlag)  //Error messages - explainations of common problems a user might have if they assemble the HUD or add their own animations
                {
                    if(nItemNo == 0)
                    {
